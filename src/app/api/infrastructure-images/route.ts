...
export async function GET(req: NextRequest) {
  ...
  try {
    const items = await prisma.infrastructureImage.findMany({ orderBy });
    return NextResponse.json(items);
  } catch (error) {
    logServerError("api.infrastructureImages.GET", error);
    const status = isDatabaseUnavailableError(error) ? 503 : 500;
    return jsonError(status === 503 ? DATABASE_UNAVAILABLE_MESSAGE : "Unable to load infrastructure images.", status);
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const item = await prisma.infrastructureImage.create({
      data: {
    ...
    logServerError("api.infrastructureImages.POST", error);
    const status = isDatabaseUnavailableError(error) ? 503 : 400;
    return jsonError(status === 503 ? DATABASE_UNAVAILABLE_MESSAGE : "Unable to create infrastructure image.", status);
  }
}